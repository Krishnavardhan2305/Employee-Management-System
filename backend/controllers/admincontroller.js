...
export const getallorganizations = async (req, res) => {
  try {
    ...
    const organization_data = await Organization.find();
...
export const loginSuperAdmin = async (req, res) => {
  ...
  try {
      const superAdmin = await SuperAdmin.findOne({ superadminmail:superadminmail });
      // console.log(superAdmin);
      if (!superAdmin) {
          return res.status(404).json({ message: "SuperAdmin not found!" });
      }

      if (superAdmin.superadminpassword !== superadminpassword) {
          return res.status(401).json({ message: "Invalid credentials!" });
      }
      res.status(200).json({ message: "Login successful!", superAdminId: superAdmin._id });
  } catch (error) {
